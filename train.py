...
batch_size      = 4096 #number of samples in each iteration
sample_size     = 4096*5 #4096 according to the paper, total number of data points 
epochs          = 5000 # 850000 according to the paper
...
#shape of each tensor will be (4096, 16)
...
def random_bools(sample_size, n):
  ...
  temp = temp*2 - 1 #converting it to -1 and 1
  return temp.astype(np.float32)
  

def model(collection, message, key=None):
  #message has text, key has secret key
  if key is not None:
...
#combined_message is a single tensor of size 32 bits

# facilites variable sharing or isolation
  with tf.variable_scope(collection):
    #each neuron is connected to every neuron
    #relu usage because they introduce non-linearity, allowing the network to learn complex patterns
    ...
    # output - tanh as it can model a wider range of output value
...
bob_bit_error_values = []
eve_bit_error_values = []
loss_values = []
...
for i in range(epochs):
  ...
  if i%5 == 0:
    bob_bit_error_values.append(temp_alice_bob_loss)
    eve_bit_error_values.append(temp_eve_loss)
    print('  epochs: ', i, '  bob bit error: ', temp_alice_bob_loss,' + ', temp_eve_evs_loss,'   & eve bit error:', temp_eve_loss)

import matplotlib.pyplot as plt

epochs_to_plot = list(range(0, epochs, 5))

plt.figure(figsize=(12, 6))
plt.plot(epochs_to_plot, bob_bit_error_values, label='Bob Bit Error')
plt.plot(epochs_to_plot, eve_bit_error_values, label='Eve Bit Error')
plt.xlabel('Epochs')
plt.ylabel('Value')
plt.title('Bob Bit Error, Eve Bit Error')
plt.legend()
plt.grid(True)
plt.show()
